...
| 默认 iterations | `10000` | `2000` | `10000` | `15000` |

Foot 配置为 `15000 iterations`，比原有 WF 多 `50%`，与其 gait 和更广地形分布相对应。Wheel 保持原有 WF 的 `10000` 默认值。PF 当前默认只有 `2000 iterations`，因此默认训练量不能与 Foot 直接视为等预算对照；CLI 均可用 `--max_iterations` 覆盖。
...
| 地形课程 | 关闭 | 关闭 | 开启 | 开启 |
| 初始地形等级 | -- | -- | `0` | `0` |
| 训练地形网格 | -- | -- | `10 rows × 16 cols` | `10 rows × 16 cols` |
| 地形难度范围 | -- | -- | `0.0～1.0` | `0.0～1.0` |
| 随机外力 | 开启 | 开启 | 关闭 | 关闭 |

原有 WF/PF Blind-Flat 都明确把 `terrain_levels = None`。当前两个新任务继承并保留 `terrain_levels_vel` 课程项，且 terrain generator 设置 `curriculum=True`，因此训练期间地形等级会根据表现升降。
...
| 上坡 | `20%` | 坡度 `0.0～0.4` |
| 下坡 | `20%` | 坡度 `0.0～0.4` |
| 波浪 | `17.5%` | 振幅 `0.01～0.06 m`，10 个波 |
| 随机粗糙 | `17.5%` | 噪声 `0.01～0.06 m` |
| 下楼梯 | `10%` | 阶高 `0.03～0.12 m`，阶宽 `0.35 m` |

Wheel 不包含上楼梯；其地形组合偏向连续滚动、坡面和较低的下阶。
...
Wheel 取消了侧向速度命令，其前向范围与 PF 相同；Foot 保留侧向移动，但整体速度范围更保守。PF 的速度命令变化最频繁，侧向范围也最大。PF 和当前两个专家都将 `rel_heading_envs` 设为零，直接训练 yaw-rate 跟踪。

### 4.2 新增高度命令

| 项目 | 原有 WF | 原有 PF | Wheel | Foot |
|---|---|---|---|---|
| 高度命令 | 无 | 无 | `0.75～0.85 m` | `0.65～0.82 m` |
| 固定奖励目标 | `0.80 m` | `0.65 m` | -- | -- |
| 重采样间隔 | -- | -- | `5～8 s` | `5～8 s` |
| 最大变化速率 | -- | -- | `0.08 m/s` | `0.06 m/s` |
| 高度基准 | 固定世界高度 | 固定世界高度 | 局部拟合地形平面 | 局部扫描中位地面高度 |
...
|---|---:|---:|
| 重采样间隔 | 固定 `5 s` | `5～8 s` |
| frequency | `1.5～2.5 Hz` | `1.2～2.2 Hz` |
| left/right offset | `0.5` | `0.5` |
| contact duration | 固定 `0.5` | `0.45～0.60` |
| swing height | `0.10～0.20 m` | `0.08～0.20 m` |

PF 和 Foot 奖励都使用 gait 相位和接触塑形，但 Foot 允许接触持续比例变化，并增加局部地面摆动高度惩罚。Wheel 只保留这些输入以统一 schema，不用 gait reward 驱动滚动。
...
由于 Actor 从 WF 的 34 维或 PF 的 40 维输入变为 162 维，且 PF 只输出 6 维动作，原有 WF/PF checkpoint 都不能无结构转换地直接加载到 Wheel/Foot 新任务。
...
`time_out` 仍是可进行 value bootstrap 的截断；`base_contact` 仍是失败终止。双专家方案没有用更多 termination 强行限制行为，轮接触和步态支撑主要由奖励处理。
...
| Foot 合规 | gait 接触准确率、摆动高度误差、双轮同时腾空时间、实际轮速 |
